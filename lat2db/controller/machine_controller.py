...
#union function for the updates of quad and sextu
        ...
        if magnet_type in machine:
            ...
            for magnet_index, magnet in enumerate(magnet_list):
                if magnet.get("name") == request_body.updated_data.name:
                    ...
                    magnet_list.insert(magnet_index, asdict(request_body.updated_data))
                    break
            ...
            return JSONResponse(status_code=200, content={"message": f"{magnet_type.capitalize()} updated"})
